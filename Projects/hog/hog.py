...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def f(s0, s1):
        former_score = last_score  
        record = running_high
        if(who == 0):
            change = s0 - former_score
            if change > record:
                print(change, "point(s)! That's a record gain for Player 0!")
                record = change
            former_score = s0
        else:
            change = s1 - former_score
            if change > record:
                print(change, "point(s)! That's a record gain for Player 1!")
                record = change
            former_score = s1
        return announce_highest(who, former_score, record)
    return f
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    max_scoring_num = 1
    avg_func = make_averaged(roll_dice,trials_count)
    max_avg_func = avg_func(max_scoring_num, dice)
    for x in range(2, 11):
        compare = avg_func(x, dice)
        if compare > max_avg_func:
            max_scoring_num = x
            max_avg_func = compare
