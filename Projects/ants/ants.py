...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        placer, count, dist = self.place, 0, self.min_range
        while not placer.is_hive and count < self.min_range:
            placer = placer.entrance
            count += 1
        while not placer.is_hive and dist <= self.max_range:
            if placer.bees != []:
                return random_bee(placer.bees)
            placer = placer.entrance
            dist += 1
        return None
        # END Problem 3 and 4
...
# The WallAnt class
class WallAnt(Ant):
    """Does nothing but has high health"""
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.chew_countdown <= 0 and b != None:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        return True if not other.is_container and self.ant_contained == None else False
        # END Problem 8
...
class TankAnt(ContainerAnt):
    """Tank ant is a better bodyguard"""
    name = 'Tank'
    food_cost = 6
    implemented = True
    damage = 1
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        #     place = gamestate.places[p]
        #     print("debug:", place.ant)
        #     if place == 'Hive':
        #         continue
        #     if place.ant:
        #         print("debug:", place.ant)
        #         if place.ant.name == cls.name:
        #             return None
        #         if place.ant.is_container:
        #             if place.ant.ant_contained and place.ant.ant_contained.name == cls.name:
        #                 return None
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        placer = self.place.exit
        while placer != None:
            if placer.ant:    
                if placer.ant != None and placer.ant.doubled == False:
                    placer.ant.damage *= 2
                    placer.ant.doubled = True
                if placer.ant.is_container and placer.ant.ant_contained:
                    if placer.ant.ant_contained.doubled == False:
                        placer.ant.ant_contained.damage *= 2
                        placer.ant.ant_contained.doubled = True
            placer = placer.exit
        return super().action(gamestate)
