...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    para = [x for x in paragraphs if select(x)]
    if len(para) <= k:
        return ''
    return para[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def f(para):
        nopunc = para.translate(str.maketrans('', '', string.punctuation))
        words = nopunc.split()
        for x in topic:
            for word in words:
                if x == word.lower():
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0 and len(reference_words) == 0:
        ...
        return 0.0
    for x in range(min(len(reference_words), len(typed_words))):
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    diffs = [diff_function(typed_word, word, limit) for word in valid_words]
    lowest = min(diffs)
    ...
    if lowest > limit:
        ...
        return valid_words[diffs.index(lowest)] 
    # END PROBLEM 5
    ...
    def f(start, goal, lim, changes):
        if changes > lim:
            ...
            if len(start) != len(goal):
                ...
                return f(start[:short_len], goal[:short_len], lim, changes)
            if len(start) <= 1:
                if start == goal:
                    return changes
                else:
                ...
                changes += 1
                return f(start[1:], goal[1:], lim, changes)
            else:
                return f(start[1:], goal[1:], lim, changes)
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        times = [[(player[t]-player[t-1]) for t in range(1,len(player))]]
        timelist += times
    return match(words, timelist)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    timeslist = [[time(match, player, word_in) for player in player_indices] for word_in in word_indices] 
    # print("DEBUG:", "timelist:", timeslist)
    mins = [min(timeslist[i]) for i in word_indices]
    # print("DEBUG:", "mins:", mins)
    who = [timeslist[i].index(mins[i]) for i in word_indices]
    # print("DEBUG:", "who", who)
    fastlist = [[word_at(match, i) for i in word_indices if who[i] == player] for player in player_indices]
    # print("DEBUG:", "fasltlist", fastlist)
    return fastlist
