...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        return scheme_apply(scheme_eval(first, env), rest.map(lambda y: scheme_eval(y, env)), env)
        # END PROBLEM 3
# map(otherfunc(x)) try map(lambda x: otherfunc(x, something))
