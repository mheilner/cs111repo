...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def pong(i, ping_pong, direction):
        if i == n:
            return ping_pong
        if i % 8 == 0 or num_eights(i) > 0:
            return pong(i+1, ping_pong - direction, direction * -1)
        else:
            return pong(i+1, ping_pong + direction, direction)
    return pong(1,1,1)
...
def missing_digits(n):
    ...
    if startDig - 1 == nextDig:
        return missing_digits(n//10)
    else: 
        return (startDig-1)-nextDig + missing_digits(n//10)
...
def count_coins(change):
    ...
    def counter(c, bigCoin):
        if c == 0:
            ...
            with_big = counter(c - bigCoin, bigCoin)
            no_big = counter(c, descending_coin(bigCoin))
            return with_big + no_big
