...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        added = Mint.present_year - self.year - 50
        return (self.cents + added) if added > 0 else self.cents
...
def store_digits(n):
    ...
    while dig >= 10:
        ...
        counter += 1
    if n // 10 == 0:
        print("debug:","linking",  dig)
        return Link(dig)
    return Link(dig, store_digits(n % (10 ** counter)))
...
def two_list(vals, amounts):
    ...
    for i in range(len(vals)):
        for x in range(amounts[n-i]):
            linky = Link(vals[n-i], linky) 
    return linky
